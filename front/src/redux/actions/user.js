...
export const changePassword=(password,newpass)=>(dispatch)=>{
              ...
              axios.post("http://localhost:8000/api/me/password/change",{olDpassword:password,token:token,newPassword:newpass})
                .then((res)=> {
...
export const updateProfile=(newinfo)=>(dispatch)=>{
  const token=cookie.load("token")
  axios.post("http://localhost:8000/api/me/profile/update",{...newinfo,token:token})
    .then(()=> {
...
export const getProfile=()=>(dispatch)=>{
  const isa=cookie.load("token");
  axios.post("http://localhost:8000/api/me/profile",{token:isa})
  .then((res)=>{
